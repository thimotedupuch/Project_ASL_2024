...
### Load dataset and re-encode variables
```{r}
Patients <- fread("SMARTc.csv", sep = ";")

Patients <- mutate(Patients, SEX = factor(SEX), 
...
### Step 3: Visualizing AIC progression (remove this step)
```{r}
aic_values <- stepwise_cox_model$anova[, "AIC"]  # Extract AIC values

(stepwise_cox_model$anova)
```

```{r}
plot(aic_values, type = "b", xlab = "Number of Variables", ylab = "AIC", main = "AIC values for stepwise Cox model")
...
# Compute the average concordance index from cross-validation
mean_concordance_cv <- mean(cv_concordance_indices)
```



# Print the cross-validated concordance index

```{r}
print(paste("Cross-validated Concordance Index:", mean_concordance_cv))
```
...
ggplot(inclusion_frequencies, aes(x = reorder(Variable, -Proportion), y = Proportion)) +
  ...
  coord_flip() +
  labs(title = "Variable Inclusion Frequencies in Bootstrapped Cox Models",
